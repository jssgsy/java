...
public class PrintMatrixInClockWise {
     ...
     */
    @Test
    public void testPrintMatrixInClockWise() {
        int[][] arr1 = {
                {1}, {2}, {3}
        };
        int[][] arr2 = {{1, 2, 3}};
        ...
        for (int[][] arr : Arrays.asList(arr1, arr2, arr3, arr4, arr5)) {
            ...
            printMatrixInClockWise(arr, rows, columns);
            System.out.println("==================");
        }
    }
     ...
     *  2. 每圈打印时的下标是多少；
     *
     * 关于第一个步骤：经过演算发现不好找出一个具体的值说总共要打印多少圈，因此转换思路为：什么时候不需要再打圈了！(重点)
     *
     * 注意点：
     ...
     * @param columns
     */
    public void printMatrixInClockWise(int[][] arr, int rows, int columns) {
        int start = 0;
        do {
